...
<?endif?>
		<div class="bx-authform-formgroup-container">
            ...
            <?if($arParams["NOT_SHOW_LINKS"] != "Y" && $arResult["NEW_USER_REGISTRATION"] == "Y" && $arParams["AUTHORIZE_REGISTRATION"] != "Y"):?>
                <input type="button" class="btn btn-secondary" id="registerBtn" name="Login" data-link="<?=$arResult["AUTH_REGISTER_URL"]?>" value="<?=GetMessage("AUTH_REGISTER")?>" />
            <?endif?>
        </div>
	</form>
...
<script>
    $(document).ready(() => {
        ...
        if($('#personal-mob-btn').length > 0)
        {
            $('#personal-mob-btn').addClass('active')
        }
    })
</script>
